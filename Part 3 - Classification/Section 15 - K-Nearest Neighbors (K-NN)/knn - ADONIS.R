...
# Fitting the K-NN to the dataset
library(class)
y_pred = knn(train = training_set[, -3],
             test = test_set[, -3],
             cl = training_set[, 3], #aqui vai o conjunto de dados da variável dependente
             k = 5) # 5 nighbors é o padrão


# Predicting the Test set results
# FAREMOS DIFERENTE PRA K-NN prob_pred = predict(classifier, type = 'response', newdata = test_set[-3]) # response indica que o retorno deve ser em um vetor com todas as probabilidades. Test_Set - 3 para selecionar apenas as variáveis independentes.
